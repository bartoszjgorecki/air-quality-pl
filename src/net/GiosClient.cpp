...
// Wynik pracy wątku parsującego JSON: dokument, surowe body i ewentualny błąd.
...
// Funkcja wykonywana w tle: zamienia tekst odpowiedzi HTTP na dokument JSON.
JsonTaskResult parseJsonDocument(QByteArray body) {
  JsonTaskResult result;
  result.body = std::move(body);

  // Ten krok uruchamiamy poza wątkiem UI, żeby duże odpowiedzi nie zawieszały okna.
...
void dumpDebugPayload(const QString& fileName, const QByteArray& raw) {
  // Surowe payloady ułatwiają diagnostykę, gdy API zmieni format odpowiedzi.
...
// Ustawia bazowy adres API i domyślny timeout dla wszystkich żądań.
GiosClient::GiosClient(QObject* parent)
  : QObject(parent),
    m_baseUrl("https://api.gios.gov.pl/pjp-api/v1/rest"),
    m_timeoutMs(12000) {}

// Wspólna funkcja sieciowa dla wszystkich endpointów GIOS.
void GiosClient::getJson(const QUrl& url, std::function<void(const QJsonDocument&, const QByteArray&)>&& onOk) {
  // Jeden punkt wejścia dla wszystkich endpointów upraszcza obsługę błędów
  // i sprawia, że timeout oraz parsowanie JSON są realizowane wszędzie tak samo.
  ...
  // Timeout jest realizowany jawnie, bo publiczne API potrafi czasem odpowiadać wolno.
  ...
  QObject::connect(reply, &QNetworkReply::finished, this, [this, reply, onOk = std::move(onOk)]() mutable {
    ...
    // Parsowanie surowego JSON trafia do wątku roboczego i wraca callbackiem.
    auto* watcher = new QFutureWatcher<JsonTaskResult>(this);
    QPointer<GiosClient> guard(this);

    // Wynik z wątku roboczego wraca tutaj i dopiero wtedy wywoływany jest parser domenowy.
...
// Pobiera wszystkie strony listy stacji i skleja je w jedną listę dla UI.
void GiosClient::fetchStations() {
  // Endpoint stacji jest stronicowany, więc zaczynamy od pierwszej strony.
  ...
  *fetchNext = [this, allStations, seenIds, fetchNext](QUrl current) {
    // Rekurencyjny callback pozwala pobrać kolejne strony bez blokowania UI.
    getJson(current, [this, allStations, seenIds, fetchNext, current](const QJsonDocument& doc, const QByteArray& raw) {
      ...
      // API bywa niespójne w paginacji, więc parser zwraca informację,
      // czy to już ostatnia użyteczna strona.
...
// Publiczna wersja pobierania sensorów emituje sygnał do AppControllera.
...
// Wersja callbackowa jest używana głównie przez warstwę mapy.
void GiosClient::fetchSensorsForStation(
  ...
  // Ta forma jest wygodna wtedy, gdy kilka stacji ma być obsługiwanych równolegle.
...
// Publiczna wersja pobierania pomiarów emituje sygnał do AppControllera.
...
// Wersja callbackowa jest używana tam, gdzie trzeba pobrać wiele stacji równolegle.
void GiosClient::fetchMeasurementsForSensor(
  int sensorId,
  std::function<void(int, QString, QVariantList)> onReady,
  std::function<void(QString)> onError
) {
  // Przy pomiarach prosimy o większy rozmiar strony, bo seria może zawierać dużo punktów.
  ...
  getJson(url, [sensorId, onReady = std::move(onReady), onError = std::move(onError)](const QJsonDocument& doc, const QByteArray& raw) mutable {
    ...
    try {
      // Parser zwraca zarówno punkty serii, jak i rozpoznany kod parametru.
