...
Window {
  ...
  // Funkcje pomocnicze w QML zajmują się filtrowaniem stacji,
  // doborem widoku mapy i prezentacją warstwy kolorowania.
  ...
  function preferredMapType(types) {
    ...
    for (var i = 0; i < types.length; i++) {
      var type = types[i]
      if (type.style === MapType.StreetMap)
        return type
    }

    for (var j = 0; j < types.length; j++) {
      var fallbackType = types[j]
      if (fallbackType.style === MapType.PedestrianMap
          || fallbackType.style === MapType.CycleMap
          || fallbackType.style === MapType.GrayStreetMap)
        return fallbackType
    }
  ...
  // Filtr tekstowy działa lokalnie na już pobranej liście stacji.
  ...
  // Buduje aktualną listę widocznych stacji po uwzględnieniu tekstu i promienia.
  ...
  // Opóźnienie zapobiega wysyłaniu wielu odświeżeń mapy podczas szybkiej zmiany filtrów.
  Timer {
  ...
  GeocodeModel {
    ...
    // Po poprawnym geokodowaniu od razu zapisujemy punkt odniesienia dla promienia.
    onLocationsChanged: {
      if (count > 0) {
  ...
  Rectangle {
    ...
    Rectangle {
      ...
      ColumnLayout {
        ...
        // Pasek nagłówka pokazuje nazwę aplikacji, komunikaty statusowe i tryb pracy.
        RowLayout {
          ...
          ColumnLayout {
            ...
            GlassCard {
              ...
              ColumnLayout {
                ...
                // Sekcja stacji: pobieranie, filtrowanie tekstowe i wyszukiwanie po promieniu.
                RowLayout {
                  Layout.fillWidth: true
                  spacing: 10
                ...
                ListView {
                  ...
                  // Lista pokazuje wyłącznie stacje po aktualnym przefiltrowaniu.
                  delegate: Rectangle {
                    ...
                    MouseArea {
                      anchors.fill: parent
                      onClicked: {
                        // Wybór stacji ustawia kontekst dla listy sensorów po lewej i mapy po prawej.
            ...
            GlassCard {
              ...
              ColumnLayout {
                ...
                // Sekcja sensorów pozwala pobrać serię online i zarządzać historią lokalną.
                Text {
                  text: "Sensors"
                  color: "white"
                  font.pixelSize: 16
                ...
                ListView {
                  ...
                  // Lista sensorów zależy od aktualnie wybranej stacji.
                  delegate: Rectangle {
                ...
                RowLayout {
                  ...
                  SpinBox {
                    ...
                    onValueModified: App.applyCurrentChartRange(historyDays.value)
                  }

                  Connections {
                    target: App
                    function onChartRangeMaxDaysChanged() {
                      if (historyDays.value > App.chartRangeMaxDays)
                        historyDays.value = App.chartRangeMaxDays
                    }
                  }
                ...
                Text {
                  ...
                  text: App.currentSensorId > 0
                        ? ("Selected sensor: " + App.currentSensorId + " | selected chart range: " + historyDays.value + "/" + App.chartRangeMaxDays + " day(s)")
                        : "Select a sensor to load online measurements or local history."
                  color: "#99FFFFFF"
                  font.pixelSize: 11
                  wrapMode: Text.WordWrap
                }
                ...
                Text {
                  Layout.fillWidth: true
                  visible: App.chartRangeInfo.length > 0
                  text: App.chartRangeInfo
                  color: "#FFD37A"
                  font.pixelSize: 11
                  wrapMode: Text.WordWrap
                }
          ...
          ColumnLayout {
            ...
            GlassCard {
              ...
              ColumnLayout {
                ...
                // Karta danych przełącza się między wykresem a mapą.
                RowLayout {
                  Layout.fillWidth: true
                  spacing: 16
                ...
                StackLayout {
                  ...
                  // Pierwsza zakładka pokazuje serię czasową i pozwala ją analizować.
                  Item {
                    Layout.fillWidth: true
                    Layout.fillHeight: true
                    ...
                    ChartView {
                      ...
                      property double defaultXMin: 0
                      property double defaultXMax: 0
                      property real defaultYMin: 0
                      property real defaultYMax: 0
                      property real minYSpan: 0.5
                      property bool pointCalloutVisible: false
                      property double pointCalloutTime: 0
                      property real pointCalloutValue: 0
                      property string pointCalloutText: ""
                      property real pointCalloutX: 0
                      property real pointCalloutY: 0

                      // Interakcje wykresu są obsługiwane lokalnie w QML:
                      // zoom, pan, reset i wybór punktu.
                      function hasInteractiveData() {
                        return App.chartPoints && App.chartPoints.length > 0
                               && defaultXMax > defaultXMin
                               && defaultYMax > defaultYMin
                      }

                      function clearSelectedPoint() {
                        pointCalloutVisible = false
                        pointCalloutText = ""
                      }

                      function formatPointCalloutText(timestamp, value) {
                        return "Value: " + Number(value).toFixed(2)
                             + "\nTime: " + Qt.formatDateTime(new Date(timestamp), "dd.MM.yyyy HH:mm:ss")
                      }

                      function updateSelectedPointGeometry() {
                        if (!pointCalloutVisible)
                          return

                        var mapped = mapToPosition(Qt.point(pointCalloutTime, pointCalloutValue), series)
                        var nextX = mapped.x + 14
                        var nextY = mapped.y - pointCallout.implicitHeight - 14

                        if (nextX + pointCallout.implicitWidth > width - 8)
                          nextX = mapped.x - pointCallout.implicitWidth - 14
                        if (nextX < 8)
                          nextX = 8

                        if (nextY < 8)
                          nextY = mapped.y + 14
                        if (nextY + pointCallout.implicitHeight > height - 8)
                          nextY = Math.max(8, height - pointCallout.implicitHeight - 8)

                        pointCalloutX = nextX
                        pointCalloutY = nextY
                      }

                      function selectPoint(timestamp, value) {
                        pointCalloutTime = timestamp
                        pointCalloutValue = value
                        pointCalloutText = formatPointCalloutText(timestamp, value)
                        pointCalloutVisible = true
                        updateSelectedPointGeometry()
                      }

                      function pickPointAt(mouseX, mouseY) {
                        if (!hasInteractiveData())
                          return false

                        // Szukamy najbliższego punktu do kliknięcia tylko w aktualnie widocznym zakresie.
                        var pts = App.chartPoints
                        var bestIndex = -1
                        var bestDistance = 15 * 15

                        for (var i = 0; i < pts.length; i++) {
                          var t = pts[i]["t"]
                          var v = pts[i]["v"]
                          if (t < axX.min.valueOf() || t > axX.max.valueOf())
                            continue
                          if (v < axY.min || v > axY.max)
                            continue

                          var mapped = mapToPosition(Qt.point(t, v), series)
                          var dx = mapped.x - mouseX
                          var dy = mapped.y - mouseY
                        ...
                        selectPoint(pts[bestIndex]["t"], pts[bestIndex]["v"])
                        return true
                      }

                      function clampXRange(newMin, newMax) {
                        var defaultSpan = defaultXMax - defaultXMin
                        var span = newMax - newMin
                        if (defaultSpan <= 0 || span >= defaultSpan)
                          return { min: defaultXMin, max: defaultXMax }

                        if (newMin < defaultXMin) {
                          newMax += defaultXMin - newMin
                          newMin = defaultXMin
                        }
                        if (newMax > defaultXMax) {
                          newMin -= newMax - defaultXMax
                          newMax = defaultXMax
                        }
                      ...
                      function clampYRange(newMin, newMax) {
                        var defaultSpan = defaultYMax - defaultYMin
                        var span = newMax - newMin
                        if (defaultSpan <= 0 || span >= defaultSpan)
                          return { min: defaultYMin, max: defaultYMax }

                        if (newMin < defaultYMin) {
                          newMax += defaultYMin - newMin
                          newMin = defaultYMin
                        }
                        if (newMax > defaultYMax) {
                          newMin -= newMax - defaultYMax
                          newMax = defaultYMax
                        }
                      ...
                      function resetView() {
                        if (!hasInteractiveData())
                          return
                        axX.min = new Date(defaultXMin)
                        axX.max = new Date(defaultXMax)
                        axY.min = defaultYMin
                        axY.max = defaultYMax
                        updateSelectedPointGeometry()
                      }
                      ...
                      function panBy(deltaX, deltaY) {
                        ...
                        var currentXMin = axX.min.valueOf()
                        var currentXMax = axX.max.valueOf()
                        var currentYMin = axY.min
                        var currentYMax = axY.max
                        var xSpan = currentXMax - currentXMin
                        var ySpan = currentYMax - currentYMin

                        var nextX = clampXRange(
                          currentXMin - (deltaX / plot.width) * xSpan,
                          currentXMax - (deltaX / plot.width) * xSpan
                        )
                        var nextY = clampYRange(
                          currentYMin + (deltaY / plot.height) * ySpan,
                          currentYMax + (deltaY / plot.height) * ySpan
                        )

                        axX.min = new Date(nextX.min)
                        axX.max = new Date(nextX.max)
                        axY.min = nextY.min
                        axY.max = nextY.max
                        updateSelectedPointGeometry()
                      }

                      function zoomAt(angleDeltaY, posX, posY) {
                        if (!hasInteractiveData() || angleDeltaY === 0)
                          return
                        ...
                        var zoomFactor = angleDeltaY > 0 ? 0.82 : 1.22
                        var currentXMin = axX.min.valueOf()
                        var currentXMax = axX.max.valueOf()
                        var currentYMin = axY.min
                        var currentYMax = axY.max
                        var currentXSpan = currentXMax - currentXMin
                        var currentYSpan = currentYMax - currentYMin

                        var nextXSpan = currentXSpan * zoomFactor
                        var nextYSpan = currentYSpan * zoomFactor
                        var minXSpan = 15 * 60 * 1000
                        var minYSpanValue = Math.max(minYSpan, (defaultYMax - defaultYMin) * 0.03)

                        if (nextXSpan < minXSpan || nextYSpan < minYSpanValue)
                          return

                        var clampedX = Math.max(plot.x, Math.min(posX, plot.x + plot.width))
                        var clampedY = Math.max(plot.y, Math.min(posY, plot.y + plot.height))
                        var xRatio = (clampedX - plot.x) / plot.width
                        var yRatio = 1 - ((clampedY - plot.y) / plot.height)

                        var xAnchor = currentXMin + currentXSpan * xRatio
                        var yAnchor = currentYMin + currentYSpan * yRatio

                        var nextX = clampXRange(
                          xAnchor - nextXSpan * xRatio,
                          xAnchor + nextXSpan * (1 - xRatio)
                        )
                        var nextY = clampYRange(
                          yAnchor - nextYSpan * yRatio,
                          yAnchor + nextYSpan * (1 - yRatio)
                        )
                      ...
                      function refresh() {
                        ...
                        // Po każdej zmianie danych wyznaczamy nowy domyślny zakres osi.
                        var minV = 1e9
                        var maxV = -1e9
                      ...
                      MouseArea {
                        ...
                        hoverEnabled: true
                        enabled: App.chartPoints.length > 0
                        ...
                        onPositionChanged: (mouse) => {
                          if (!(mouse.buttons & Qt.LeftButton))
                            return
                          var movedX = mouse.x - pressMouseX
                          var movedY = mouse.y - pressMouseY
                          if (!movedDuringDrag && (Math.abs(movedX) > 4 || Math.abs(movedY) > 4))
                            movedDuringDrag = true
                          chart.panBy(mouse.x - lastMouseX, mouse.y - lastMouseY)
                          lastMouseX = mouse.x
                          lastMouseY = mouse.y
                        }

                        onReleased: (mouse) => {
                          // Krótki klik pokazuje tooltip punktu, a przeciągnięcie przesuwa wykres.
                          if (!movedDuringDrag)
                            chart.pickPointAt(mouse.x, mouse.y)
                        }

                        onWheel: (wheel) => {
                          chart.zoomAt(wheel.angleDelta.y, wheel.x, wheel.y)
                          wheel.accepted = true
                        }
                      ...
                      Rectangle {
                        id: pointCallout
                        visible: chart.pointCalloutVisible
                        x: chart.pointCalloutX
                        y: chart.pointCalloutY
                        radius: 10
                        color: "#CC0B1016"
                        border.color: "#44FFFFFF"
                        border.width: 1
                        z: 30
                        implicitWidth: Math.max(156, calloutText.implicitWidth + 20)
                        implicitHeight: calloutText.implicitHeight + 16

                        Text {
                          id: calloutText
                          anchors.fill: parent
                          anchors.margins: 10
                          text: chart.pointCalloutText
                          color: "#F2FBFF"
                          font.pixelSize: 11
                          wrapMode: Text.WordWrap
                        }
                  ...
                  // Druga zakładka pokazuje te same stacje w układzie przestrzennym.
                  Item {
                    Layout.fillWidth: true
                    Layout.fillHeight: true
                    ...
                    Map {
                      ...
                      activeMapType: preferredMapType(supportedMapTypes)
                      property var startCentroid: QtPositioning.coordinate()

                      // Obsługa mapy przypomina zachowanie znane z map internetowych.
                      PinchHandler {
                        id: mapPinch
                        target: null
                        onActiveChanged: if (active) {
                          stationMap.startCentroid = stationMap.toCoordinate(mapPinch.centroid.position, false)
                        }
                        onScaleChanged: (delta) => {
                          stationMap.zoomLevel += Math.log2(delta)
                          stationMap.alignCoordinateToPoint(stationMap.startCentroid, mapPinch.centroid.position)
                        }
                      ...
                      MapItemView {
                        ...
                        // Każda stacja jest markerem kolorowanym zgodnie z aktualną warstwą pomiarową.
                        delegate: MapQuickItem {
                          ...
                          sourceItem: Item {
                            ...
                            Rectangle {
                              anchors.centerIn: parent
                              width: selectedStationId === station.id ? 18 : 22
                              height: width
                              radius: width / 2
                            ...
                            TapHandler {
                              ...
                              onTapped: {
                                // Kliknięcie markera działa tak samo jak kliknięcie stacji na liście.
                    ...
                    Rectangle {
                      ...
                      Column {
                        ...
                        // Ten panel zbiera komunikaty o filtrach i stanie warstwy mapy.
                        Text {
                          width: parent.width
                          visible: stationFilterStatusText().length > 0
                          text: stationFilterStatusText()
                    ...
                    Rectangle {
                      ...
                      Column {
                        ...
                        // Legenda pokazuje, jak interpretować kolory markerów na mapie.
                        Text {
                          width: parent.width
                        ...
                        Rectangle {
                          ...
                          gradient: Gradient {
                            ...
                            GradientStop { position: 0.0; color: "#2E7CFF" }
                            GradientStop { position: 0.2; color: "#00C8FF" }
                            GradientStop { position: 0.4; color: "#38D97A" }
                            GradientStop { position: 0.6; color: "#B8E64C" }
                            GradientStop { position: 0.8; color: "#FFB347" }
            ...
            GlassCard {
              ...
              ColumnLayout {
                ...
                // Karta analizy pokazuje statystyki policzone po stronie C++.
                Text {
                  text: "Analysis"
                  color: "white"
                  font.pixelSize: 16
