...
8. In the chart view, use the mouse wheel to zoom, drag to pan, and double-click to reset the visible range.
9. Click a chart point to display its exact value and timestamp.
10. Click `Save to Local DB` to persist the current series in the local JSON database.
11. Use `History range` and `Load Local History` to display previously saved data when needed.
